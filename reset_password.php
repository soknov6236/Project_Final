...
if (isset($_GET['token']) && !empty($_GET['token'])) {
    ...
    try {
        ...
        $stmt = $conn->prepare("SELECT id, username FROM users WHERE reset_token = ? AND token_expiry > NOW()");
        if (!$stmt) {
    ...
    if (empty($password) || empty($confirm_password)) {
        ...
        try {
            ...
            if ($stmt->execute()) {
                ...
                $valid_token = false; // Invalidate the token after successful reset
...
<html lang="en" data-pc-preset="preset-1" data-pc-sidebar-caption="true" data-pc-direction="ltr" dir="ltr" data-pc-theme="light">
  ...
  <head>
    ...
    <style>
      ...
      .strength-fair { width: 50%; background-color: #fd7e14; }
      .strength-good { width: 75%; background-color: #ffc107; }
                ...
                <?php elseif ($valid_token): ?>
                  ...
                  <form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"] . '?token=' . $token); ?>">
                    ...
                    <div class="mt-4 text-center">
                      <button type="submit" class="btn btn-primary mx-auto shadow-2xl">Reset Password</button>
                    </div>
                  </form>
                <?php else: ?>
                  <div class="text-center">
                    <p class="text-muted mb-4">Unable to reset password. Please request a new reset link.</p>
